...
- **Models:** Gemini via the Gemini Developer API (AI Studio key, has a free tier). `/api/chat` uses **Gemini 2.5 Flash** (`gemini-2.5-flash`); `/api/end-session` uses **Flash-Lite** (`gemini-2.5-flash-lite`); `/api/export-log` reuses `MODEL_CHAT` (Flash). Chat is on Flash because Flash-Lite misreads the ~40k-token knowledge (e.g. confused which treasure chests need the thief's vs magic key) and asserted wrong facts; Flash grounds reliably. end-session only summarizes the conversation into state JSON (no knowledge lookup), so Flash-Lite is enough. Model ids + the client live in `src/lib/gemini.ts`; to trade accuracy for cost, drop `MODEL_CHAT` back to Flash-Lite there. Thinking is disabled (`thinkingConfig.thinkingBudget: 0`) for tempo, and `safetySettings` are set to `BLOCK_NONE` so game content (battles, monsters) isn't filtered to an empty reply. (`/api/tts` is not a Gemini endpoint — it calls Google Cloud TTS.)
